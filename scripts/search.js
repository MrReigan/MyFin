...
function debounce(func, delay) {
  ...
  function handleSearch() {
    ...
    // Управляем отступом контейнера
    if (hasResults) {
      contentContainer.classList.add('has-results'); // Добавляем класс для отступа
    } else {
      contentContainer.classList.remove('has-results'); // Убираем класс для отступа
    }
  }
